...
### Тесты:
![](screenshots/3.3(1).png) ![](screenshots/3.3(2).png)
...
### Алгоритм Решения:
Создан новый класс NameEnhanced с улучшенной логикой  
Реализованы три конструктора с разными наборами параметров  
Формат вывода изменен на "Имя Фамилия Отчество"  
Сохранена обратная совместимость с оригинальным классом Name
...
### Алгоритм Решения:
- Создан класс PeopleEnhanced с четырьмя перегруженными конструкторами  
- Реализован приватный метод inheritFromFather() для наследования фамилии и отчества  
- Формат вывода изменен согласно требованиям  
- Использована цепочка конструкторов через this() для уменьшения дублирования кода  
...
- Может помяукать Nраз, что приводит к выводу на экран следующего текста:  
“Имя: мяумяу-…-мяу!”, где количество “мяу”равно N.  
Создайте кота по имени “Барсик”, и затем пусть он помяукает сначала один раз, а затем три раза.  
### Алгоритм Решения:
Создан класс Cat с полем name  
Реализованы методы:  
- meow() - мяуканье один раз  
- meow(int times) - мяуканье N раз с разделителем "-"  
Метод toString() выводит информацию в формате "кот: Имя"
